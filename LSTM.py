...
class LSTM_composer(torch.nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers):
        super(LSTM_composer, self).__init__()
    ...
    def forward(self, inputs, hidden, Temperature):
        batch, tmp = inputs.shape[0], [0]*output_size
        ...
        #outputs = self.log_softmax(self.out2tag(outputs.view(inputs.shape[0], inputs.shape[1], self.hidden_size)))
    ...
    def init_hidden(self, num_layers, batch, hidden_size):
        ...
        autograd.Variable(torch.zeros(num_layers, batch, hidden_size)).type(torch.FloatTensor).cuda())


LR = 0.001   # when using Adam, default lr = 0.001
epochs = 40
seq_len = 100
batch = 1
input_size = 93
...
model = LSTM_composer(input_size, hidden_size, output_size, num_layers)
model = model.to(computing_device)
print("Model on CUDA?", next(model.parameters()).is_cuda)
loss_function = nn.NLLLoss()
optimizer = optim.Adam(model.parameters())
(h, c) = model.init_hidden(num_layers, batch, hidden_size)
...
for epoch_cnt in range(epochs):
    ...
    for batch_cnt in range(train_leng):
        ...
        outputs, (h, c) = model(inputs, (h, c), softmax_temperature)
    ...
    print("Epoch:", epoch_cnt + 1, " Train loss:", train_loss/train_leng, end = ', ')
    ...
    with torch.no_grad():
        if epoch_cnt % 1 == 0:
            (h2, c2) = model.init_hidden(num_layers, batch, hidden_size)
            for batch_cnt2 in range(val_leng):
                inputs, targets, targets_C = make_chunks(val_data, batch, seq_len, seq_len*batch*batch_cnt2)
                inputs, targets_C = inputs.to(computing_device), targets_C.to(computing_device)
                outputs, (h2, c2) = model(inputs, (h2, c2), softmax_temperature)
                loss = loss_function(outputs.reshape(batch*seq_len, 93), targets_C.reshape(batch*seq_len,))
                val_loss += float(loss.item())
                h2.detach()
                c2.detach()
                del loss, inputs, targets, targets_C, outputs
            print("Valid loss:", val_loss/val_leng)
...
test_loss, test_leng = 0.0, int(len(test_data)/(batch*seq_len))
model.eval()
with torch.no_grad():
    (h2, c2) = model.init_hidden(num_layers, batch, hidden_size)
    for batch_cnt3 in range(test_leng):
        inputs, targets, targets_C = make_chunks(test_data, batch, seq_len, seq_len*batch*batch_cnt3)
        inputs, targets_C = inputs.to(computing_device), targets_C.to(computing_device)
        outputs, (h2, c2) = model(inputs, (h2, c2), softmax_temperature)
        loss = loss_function(outputs.reshape(batch*seq_len, 93), targets_C.reshape(batch*seq_len,))
        ...
        c2.detach()
        del loss, inputs, targets, targets_C, outputs
    print("Test loss:", test_loss/test_leng)
...
with torch.no_grad():
    ...
    while len(music_txt) < target_len:
        ...
        #input_txt = input_txt[1:] + output_txt[-1]
